...
inline OpenVRHookManagerAPI* RequestOpenVRHookManagerObject()
{
	...
	if (skyrimVRToolsModule != nullptr)
	{
		...
		{
			spdlog::error("Failed to get address of function GetVRHookManager from skyrimvrtools.dll. Is your skyrimvrtools.dll out of date?");
		}
	}
	else
	{
		spdlog::error("Failed to load skyrimvrtools.dll in RequestOpenVRHookManagerObject()");
	}
